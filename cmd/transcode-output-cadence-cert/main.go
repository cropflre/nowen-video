...
func main() {
	...
	for _, report := range matrix.Cases {
		...
		fmt.Fprintf(
			os.Stderr,
			"case=%s mode=%s source_frames=%d startup_in=%d startup_out=%d startup_delta=%d startup_min_us=%d startup_max_us=%d startup_dominant_us=%d startup_raw_variable=%t startup_material_variable=%t startup_outliers=%d continuation_in=%d continuation_out=%d continuation_delta=%d continuation_min_us=%d continuation_max_us=%d continuation_dominant_us=%d continuation_raw_variable=%t continuation_material_variable=%t continuation_outliers=%d duplicate_pts=%d non_monotonic_pts=%d preservation=%s discontinuity_required=%t\n",
			report.Case.ID,
