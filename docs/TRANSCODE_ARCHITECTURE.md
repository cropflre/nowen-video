...
当前已落地 direct、remux、smart_remux、runtime HLS，以及数据库 Job Claim / Lease、Priority + FIFO 调度、重启续队和优雅关闭。`startup_stream` 与长期预处理迁移属于后续阶段，不应通过伪造缓存或短路播放状态代替。
...
每次成功领取生成独立 `lease_token`。进入运行态、续租、优雅释放和提交终态都必须匹配该令牌。旧 Worker 丢失或释放 Lease 后，即使进程延迟返回成功，也不能覆盖新 Worker 或恢复器提交的状态。
...
当前不对运行中的 FFmpeg 做强制抢占；这需要 Attempt 隔离输出、可续跑点和显式抢占策略后再实现。
...
服务重启或 Lease 回收后，Attempt 编号从数据库历史最大值继续递增，不会重新从 1 开始。
...
- Lease 所有权约束运行态、释放和终态写入。
- queued、过期 Lease 和升级前无 Lease记录的重启续队。
- 坏载荷隔离，不阻塞低优先级有效任务。
- 跨重启 Attempt 编号连续。
- 交互 100、重试 70、后台 20 的持久优先级分类。
- Active Key 复用时的排队任务优先级原子提升。
- 优雅关闭停止新 Claim；超时后由 Lease 所有者安全重新排队。
- 管理统计返回数据库队列深度、`scheduler=database_priority_fifo`、轮询周期和 Lease 时长。
- 排队和运行中任务的持久取消语义。
