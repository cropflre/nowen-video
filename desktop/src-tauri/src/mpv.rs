//! Nowen Video Desktop 2.0 原生播放器核心。
//!
//! 桌面正式运行时只保留 libmpv 一个播放内核。浏览器播放属于 Web 平台；
//! Rust 层不再启动外部 mpv.exe，也不再维护第二套播放器进程生命周期。
...
pub struct MpvManager {
    ...
    sessions: HashMap<String, embed::EmbeddedMpv>,
}
...
impl MpvManager {
    ...
    /// 兼容旧 IPC 的可用性字段；Desktop 2.0 中 available 与 embed_available 等价。
    pub fn is_available(&self) -> bool {
        self.is_embed_available()
    }
    ...
    /// Desktop 2.0 已移除外部 mpv 进程模式。
    /// 该方法仅用于旧 IPC 过渡期保持编译，调用时明确拒绝。
    pub fn play(&mut self, _session_id: &str, _url: &str, _options: PlayOptions) -> Result<()> {
        Err(anyhow!("Desktop 2.0 已移除外部 mpv 播放模式，请使用原生 Player Core"))
    }

    pub fn stop(&mut self, session_id: &str) {
        #[cfg(feature = "embed-mpv")]
        if self.sessions.remove(session_id).is_some() {
            log::info!("已释放原生播放器会话: {}", session_id);
        }
    }
    ...
    pub fn stop_all(&mut self) {
        #[cfg(feature = "embed-mpv")]
        {
            if !self.sessions.is_empty() {
                log::info!("释放全部原生播放器会话: {}", self.sessions.len());
            }
            self.sessions.clear();
        }
    }
    ...
    pub fn play_embedded(
        ...
        let player = embed::EmbeddedMpv::new(wid, &options)?;
        player.loadfile(url)?;
    ...
    pub fn play_embedded(
        ...
        Err(anyhow!("当前构建未启用 Desktop Player Core（embed-mpv）"))
    }

    #[cfg(feature = "embed-mpv")]
    pub fn send_embed_command(&mut self, session_id: &str, command: &str, args: &[String]) -> Result<()> {
        self.session(session_id)?.command(command, args)
    }

    #[cfg(not(feature = "embed-mpv"))]
    pub fn send_embed_command(&mut self, _session_id: &str, _command: &str, _args: &[String]) -> Result<()> {
    ...
    #[cfg(not(feature = "embed-mpv"))]
    pub fn set_embed_property(&mut self, _session_id: &str, _name: &str, _value: &str) -> Result<()> {
        Err(anyhow!("当前构建未启用 Desktop Player Core"))
    }

    /// Anime4K 延后到 Desktop 2.1。保留 Rust 方法仅用于旧 IPC 过渡期编译。
    pub fn set_embed_anime4k(&mut self, _session_id: &str, _level: &str) -> Result<()> {
        Err(anyhow!("Anime4K 已从 Desktop 2.0 首发范围移除，将在后续版本重新接入"))
    }

    #[cfg(feature = "embed-mpv")]
    pub fn get_embed_video_info(&self, session_id: &str) -> Result<EmbedVideoInfo> {
        Ok(self.session(session_id)?.get_video_info())
    }
    ...
    #[cfg(feature = "embed-mpv")]
    fn session(&self, session_id: &str) -> Result<&embed::EmbeddedMpv> {
        self.sessions
            .get(session_id)
            .ok_or_else(|| anyhow!("原生播放器会话不存在: {}", session_id))
    }
}
...
mod embed {
    ...
    impl EmbeddedMpv {
        pub fn new(wid: i64, options: &PlayOptions) -> Result<Self> {
            ...
            map_err(mpv.set_property("wid", wid), "绑定播放器原生窗口失败")?;
