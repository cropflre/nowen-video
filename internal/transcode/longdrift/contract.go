...
func BuildCandidateSummary(runs []RunEvidence) CandidateSummary {
	...
	for metric := 0; metric < 4 && len(metrics) > 0; metric++ {
		...
		for _, values := range metrics[1:] {
			if values[metric] < minimum { minimum = values[metric] }
			if values[metric] > maximum { maximum = values[metric] }
		}
...
func BuildCandidateComparison(a, b CandidateEvidence) CandidateComparison {
	...
	for index := range a.Runs {
		...
		for _, pair := range [][2][]CheckpointEvidence{{left.Video.Checkpoints, right.Video.Checkpoints}, {left.Audio.Checkpoints, right.Audio.Checkpoints}} {
			if len(pair[0]) != len(pair[1]) { return result }
			for checkpoint := range pair[0] {
				if pair[0][checkpoint].TargetMicros != pair[1][checkpoint].TargetMicros { return result }
...
func (c Contract) ValidateFor(spec transcodecorpus.Spec, manifest transcodecorpus.Manifest) error {
	if c.SchemaVersion != SchemaVersion { return fmt.Errorf("unsupported long-duration drift schema %q", c.SchemaVersion) }
	if err := manifest.ValidateFor(spec); err != nil { return err }
	...
	if strings.TrimSpace(c.CertificationFFmpegVersion) == "" || strings.TrimSpace(c.CertificationFFprobeVersion) == "" { return fmt.Errorf("certification toolchain is incomplete") }
	...
	if !ok { return fmt.Errorf("long-duration source case is missing") }
	if c.Source.CaseID != SourceCaseID || c.Source.RelativePath != asset.RelativePath || c.Source.SHA256 != asset.SHA256 || c.Source.SizeBytes != asset.SizeBytes { return fmt.Errorf("long-duration source identity differs from manifest") }
	assetHash, err := CanonicalHash(asset)
	if err != nil { return err }
	if c.Source.AssetEvidenceHash != assetHash || !isSHA256(c.Source.AssetEvidenceHash) { return fmt.Errorf("long-duration asset identity is invalid") }
	if caseSpec.Source.Audio.SampleRate != 44_100 || caseSpec.Source.Video.BFrames != 3 { return fmt.Errorf("long-duration source profile is invalid") }
	if len(c.Candidates) != 2 { return fmt.Errorf("long-duration matrix must contain two candidates") }
	for index, candidate := range c.Candidates {
		want := []struct{id, tb string}{{transcodetimebase.CandidateAVTB, "1/1000000"}, {transcodetimebase.Candidate90K, "1/90000"}}[index]
		if candidate.ID != want.id || candidate.EncoderTimeBase != want.tb { return fmt.Errorf("long-duration candidate order is invalid") }
		if err := candidate.Validate(); err != nil { return fmt.Errorf("validate candidate %s: %w", candidate.ID, err) }
	}
	if c.Comparison != BuildCandidateComparison(c.Candidates[0], c.Candidates[1]) || !c.Comparison.Equivalent { return fmt.Errorf("long-duration candidates diverged") }
	if c.SeamlessAllowed || !c.DiscontinuityRequired { return fmt.Errorf("long-duration evidence cannot authorize seamless playback") }
	return nil
}

func (c CandidateEvidence) Validate() error {
	if len(c.Runs) != RepeatCount { return fmt.Errorf("candidate has %d repeats, want %d", len(c.Runs), RepeatCount) }
	for index, run := range c.Runs {
		if err := run.Validate(index+1); err != nil { return err }
	}
	if c.Summary != BuildCandidateSummary(c.Runs) || !c.Summary.Stable { return fmt.Errorf("candidate summary is unstable") }
	return nil
}

func (r RunEvidence) Validate(ordinal int) error {
	if r.Ordinal != ordinal { return fmt.Errorf("run ordinal is invalid") }
	for label, value := range map[string]string{"command": r.CommandHash, "manifest": r.ManifestSHA256, "attestation": r.AttestationHash} {
		if !isSHA256(value) { return fmt.Errorf("%s identity is invalid", label) }
	}
	if strings.TrimSpace(r.AttestationVersion) == "" || r.SegmentCount < 890 || r.SegmentCount > 910 { return fmt.Errorf("run attestation or segment count is invalid") }
	if err := r.Video.Validate("video"); err != nil { return err }
	if err := r.Audio.Validate("audio"); err != nil { return err }
	if r.FinalAVSkewMicros != r.Video.EndMicros-r.Audio.EndMicros || abs64(r.FinalAVSkewMicros) > AVSkewToleranceMicros { return fmt.Errorf("final A/V skew is invalid") }
	return nil
}

func (s StreamEvidence) Validate(kind string) error {
	if s.Kind != kind || strings.TrimSpace(s.TimeBase) == "" || s.PacketCount <= 0 { return fmt.Errorf("%s stream identity is invalid", kind) }
	if abs64(s.StartMicros) > StartToleranceMicros || s.EndMicros <= s.StartMicros || s.DurationMicros != s.EndMicros-s.StartMicros || s.EndErrorMicros != s.EndMicros-DurationMicros || abs64(s.EndErrorMicros) > EndToleranceMicros { return fmt.Errorf("%s stream duration evidence is invalid", kind) }
	expectedCount := int(DurationMicros/CheckpointMicros) + 1
	if len(s.Checkpoints) != expectedCount { return fmt.Errorf("%s checkpoint count is invalid", kind) }
	lastPresentation := int64(-1)
	for index, checkpoint := range s.Checkpoints {
		wantTarget := int64(index) * CheckpointMicros
		if checkpoint.TargetMicros != wantTarget || checkpoint.ErrorMicros != checkpoint.PresentationMicros-checkpoint.TargetMicros || abs64(checkpoint.ErrorMicros) > CheckpointToleranceMicros || checkpoint.PresentationMicros < lastPresentation { return fmt.Errorf("%s checkpoint %d is invalid", kind, index) }
...
func Identity(contract Contract, spec transcodecorpus.Spec, manifest transcodecorpus.Manifest) (version, hash, canonical string, err error) {
	if err := contract.ValidateFor(spec, manifest); err != nil { return "", "", "", err }
...
func findSource(spec transcodecorpus.Spec, manifest transcodecorpus.Manifest) (transcodecorpus.CaseSpec, transcodecorpus.AssetEvidence, bool) {
	...
	for _, asset := range manifest.Assets { assets[asset.CaseID] = asset }
	for _, caseSpec := range spec.Cases {
		if caseSpec.ID == SourceCaseID { asset, ok := assets[caseSpec.ID]; return caseSpec, asset, ok }
	}
...
func CheckpointTargets() []int64 {
	...
	for value := int64(0); value <= DurationMicros; value += CheckpointMicros { result = append(result, value) }
	return result
}

func SortCheckpoints(values []CheckpointEvidence) { sort.Slice(values, func(i, j int) bool { return values[i].TargetMicros < values[j].TargetMicros }) }
func abs64(value int64) int64 { if value < 0 { return -value }; return value }
func max64(a, b int64) int64 { if b > a { return b }; return a }
