...
VERSION_NAME="1.2.9"
KEYSTORE_PATH=""
KEY_ALIAS="${ANDROID_KEY_ALIAS:-${ANDROID_V2_KEY_ALIAS:-}}"
REPOSITORY="cropflre/nowen-video"
REPORT_PATH=""
EXPECTED_FINGERPRINT="${ANDROID_EXPECTED_CERTIFICATE_SHA256:-${ANDROID_V2_CERTIFICATE_SHA256:-}}"
...
# Official post-takeover environment names. Historical V2 names are accepted as
# read-only compatibility fallbacks, but new configuration is always written
# using ANDROID_* names.
ANDROID_KEYSTORE_PASSWORD="${ANDROID_KEYSTORE_PASSWORD:-${ANDROID_V2_KEYSTORE_PASSWORD:-}}"
ANDROID_KEY_PASSWORD="${ANDROID_KEY_PASSWORD:-${ANDROID_V2_KEY_PASSWORD:-}}"
...
Required environment variables for the historical production keystore:
  ANDROID_KEYSTORE_PASSWORD
  ANDROID_KEY_PASSWORD

Options:
  --version VERSION                 Android versionName (default: 1.2.9)
  --keystore PATH                   Historical Android V1 release keystore path
  --alias ALIAS                     Private-key alias (or ANDROID_KEY_ALIAS)
  --expected-fingerprint SHA256     Optional expected signing certificate SHA-256
  --repository OWNER/REPO           GitHub repository
  --report PATH                     Write a non-sensitive JSON preflight report
  --set-github-secrets              Verify V1 identity, then configure four official Android Secrets
...
Production rule:
  The current Android client replaces V1 in-place under com.nowen.video.
  DO NOT generate a new production keystore. Production signing must use the
  same private key that signed the published v1.2.5 APK. When
  --set-github-secrets is used, this script compares the supplied keystore to
  that published APK before writing any repository secret.

Secrets written by --set-github-secrets:
  ANDROID_KEYSTORE_BASE64
  ANDROID_KEYSTORE_PASSWORD
  ANDROID_KEY_ALIAS
  ANDROID_KEY_PASSWORD

The certificate fingerprint is intentionally not stored as a Secret. CI derives
its trust anchor directly from the published V1 APK on every production build.
USAGE
}
...
verify_historical_v1_signer() {
  ...
  GH_TOKEN="${GH_TOKEN:-${GITHUB_TOKEN:-}}" gh release download "$LEGACY_RELEASE_TAG" \
    --repo "$REPOSITORY" \
    --pattern "$LEGACY_APK_NAME" \
    --dir "$temp_dir" >/dev/null
  ...
  [[ "$candidate_fingerprint" == "$legacy_fingerprint" ]] || \
    fail "keystore is NOT the Android V1 production signer; refusing production takeover"
  printf '%s\n' "$legacy_fingerprint"
}
...
run_self_test() {
  ...
  temp_dir="$(mktemp -d)"
  keystore="$temp_dir/android-test.jks"
...
configure_github_secrets() {
  local keystore="$1" alias="$2" fingerprint="$3" historical_fingerprint
  require_command gh
  require_command base64

  historical_fingerprint="$(verify_historical_v1_signer "$fingerprint")"
  gh auth status >/dev/null
  gh repo view "$REPOSITORY" >/dev/null

  base64 < "$keystore" | tr -d '\r\n' | \
    gh secret set ANDROID_KEYSTORE_BASE64 --repo "$REPOSITORY"
  printf '%s' "$ANDROID_KEYSTORE_PASSWORD" | \
    gh secret set ANDROID_KEYSTORE_PASSWORD --repo "$REPOSITORY"
  printf '%s' "$alias" | \
    gh secret set ANDROID_KEY_ALIAS --repo "$REPOSITORY"
  printf '%s' "$ANDROID_KEY_PASSWORD" | \
    gh secret set ANDROID_KEY_PASSWORD --repo "$REPOSITORY"

  printf 'Configured four Android production signing secrets.\n'
  printf 'Historical V1 signer verified: %s\n' "$historical_fingerprint"
  printf 'The certificate fingerprint is derived from the published V1 APK in CI and is not stored as a secret.\n'
}
...
payload = {
    ...
    "trust_anchor": {"release_tag": "v1.2.5", "apk": "nowen-video-android-1.2.5.apk"},
...
[[ -n "$KEY_ALIAS" ]] || fail "--alias or ANDROID_KEY_ALIAS is required"
...
if [[ -n "$EXPECTED_FINGERPRINT" ]]; then
  ...
  [[ "$FINGERPRINT" == "$EXPECTED_FINGERPRINT" ]] || \
    fail "keystore certificate SHA-256 does not match expected fingerprint"
fi
