...
import { desktop, type PlayOptions, type MpvVideoInfo } from './bridge'
...
function MpvEmbedPlayerInner(
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      if (!result) {
        const message = '原生播放器启动失败，请检查桌面播放器运行时是否完整。'
        setError(message)
        onError?.(message)
        return
      }
  ...
  // 当前 Rust Player Core 尚未切到 libmpv observe_property 事件泵；
  // 这里作为过渡层统一同步 Zustand。后续事件化时只需替换这一处。
  useEffect(() => {
