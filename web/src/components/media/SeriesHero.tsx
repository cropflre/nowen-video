...
export default function SeriesHero({
  ...
  return (
    <section className="nv-detail-hero nv-series-hero relative overflow-hidden border-b border-[var(--nv-border-subtle)] bg-[var(--nv-bg-canvas)]">
      ...
      <div className="nv-detail-hero-inner nv-series-hero-inner relative mx-auto grid w-full max-w-[var(--nv-content-max)] items-center gap-6 px-[var(--nv-page-gutter)] sm:grid-cols-[11rem_minmax(0,1fr)] lg:grid-cols-[12rem_minmax(0,1fr)] lg:gap-8">
        <div className="hidden sm:block">
          <div className="nv-detail-poster nv-series-poster aspect-[2/3] overflow-hidden rounded-[var(--nv-radius-card)] border border-[var(--nv-border-default)] bg-[var(--nv-bg-poster)] shadow-[var(--nv-shadow-card)]">
            {series.poster_path ? (
              <img
        ...
        <div className="nv-series-hero-content min-w-0">
          <div className="nv-series-hero-badges mb-3 flex flex-wrap items-center gap-1.5">
            <Tag>剧集</Tag>
          ...
          <h1 className="nv-series-hero-title text-[var(--nv-text-primary)]">{series.title}</h1>
          {series.orig_title && series.orig_title !== series.title && (
            <p className="nv-series-hero-original mt-2 max-w-3xl text-sm text-[var(--nv-text-secondary)]">{series.orig_title}</p>
          )}

          <div className="nv-series-hero-meta mt-3 flex flex-wrap items-center gap-x-2 gap-y-1.5 text-xs text-[var(--nv-text-secondary)]">
            {genres.slice(0, 4).map((genre, index) => (
              <span key={genre} className="inline-flex items-center gap-2">
                {index > 0 && <span className="text-[var(--nv-text-tertiary)]" aria-hidden="true">·</span>}
          ...
          {series.overview && (
            <p className="nv-series-hero-overview mt-4 line-clamp-2 max-w-3xl text-sm leading-6 text-[var(--nv-text-secondary)]">
              {series.overview}
            </p>
          )}

          <div className="nv-series-hero-actions mt-5 flex flex-wrap items-center gap-2">
            {playEpisode && (
